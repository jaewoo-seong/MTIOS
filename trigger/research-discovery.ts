...
export async function runResearchDiscovery(projectId: string, cyclesRemaining = 3) {
  ...
  for (const query of queries) {
    const result = await runResearchQuery({
      projectId, agendaId: campaign.agendaId, query, category: "web", language: "en",
  ...
  const response = await requestModel("worker_structured", [
    {
      ...
      content: [
        ...
        'Return JSON only: {"candidates":[{"legalName":"string","website":"string","location":"string","industry":"string","qualificationReason":"string","qualificationScore":0}]}'
      ].join("\n")
    },
